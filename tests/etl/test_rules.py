...
def test_validate_companies_ticker_length(validator):
    df = pd.DataFrame([
        {"id": "T", "company_name": "Too Short", "face_value": 1.0},  # Invalid (length 1)
        {"id": "A" * 15, "company_name": "Too Long", "face_value": 1.0},  # Invalid (length 15)
...
def test_validate_profitandloss_duplicates_and_orphans(validator):
    pl_df = pd.DataFrame([
        {"company_id": "TCS", "year": "2023-03", "sales": 100, "expenses": 80, "operating_profit": 20, "opm_percentage": 20.0},
        {"company_id": "TCS", "year": "2023-03", "sales": 100, "expenses": 75, "operating_profit": 25, "opm_percentage": 25.0}, # Duplicate (keeps last)
        {"company_id": "ORPHAN", "year": "2023-03", "sales": 100, "expenses": 80, "operating_profit": 20, "opm_percentage": 20.0} # Orphan
    ])
    valid_companies = {"TCS"}
...
def test_validate_profitandloss_opm_crosscheck(validator):
    pl_df = pd.DataFrame([
        {"company_id": "TCS", "year": "2023-03", "sales": 100, "expenses": 80, "operating_profit": 20, "opm_percentage": 10.0} # Calculated OPM is 20%, reported is 10%
    ])
...
def test_validate_profitandloss_positive_sales(validator):
    pl_df = pd.DataFrame([
        {"company_id": "TCS", "year": "2023-03", "sales": -10, "expenses": 10, "operating_profit": -20, "opm_percentage": 0.0},
        {"company_id": "PNB", "year": "2023-03", "sales": 0, "expenses": 10, "operating_profit": -10, "opm_percentage": 0.0}
    ])
...
def test_validate_balancesheet_balance_and_fixed_assets(validator):
    bs_df = pd.DataFrame([
        {"company_id": "TCS", "year": "2023-03", "equity_capital": 10, "total_assets": 100, "total_liabilities": 105, "fixed_assets": -5} # Mismatch (5% diff) & Negative fixed assets
    ])
...
def test_validate_cashflow_net_check(validator):
    cf_df = pd.DataFrame([
        {"company_id": "TCS", "year": "2023-03", "operating_activity": 100, "investing_activity": -40, "financing_activity": -20, "net_cash_flow": 10} # Reported is 10, calc is 40 (diff > 10 Cr)
    ])
