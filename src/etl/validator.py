...
class DataValidator:
    ...
    def log_failure(
        self, company_id: str, year: str, field: str, issue: str, severity: str
    ):
    ...
    def validate_companies(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if df["face_value"].isna().any():
            ...
            for co in null_cos:
                self.log_failure(
                    ...
                    "N/A",
                    "face_value",
                    "Missing face_value coerced to 1.0 (DQ warning)",
                    "WARNING",
                )
        ...
        if df["id"].nunique() != len(df):
            ...
            for dup in duplicates:
                self.log_failure(
                    dup, "N/A", "id", "Duplicate company ticker (DQ-01)", "CRITICAL"
                )
            raise ValueError(
                f"CRITICAL: Duplicate company tickers found: {list(duplicates)}. Halting load."
            )
        ...
        for idx, row in df.iterrows():
            ...
            if not (2 <= len(ticker) <= 12):
                self.log_failure(
                    ...
                    "N/A",
                    "id",
                    f"Ticker length {len(ticker)} out of range 2-12 (DQ-08)",
                    "CRITICAL",
                )
                continue  # Reject row
    ...
    def validate_profitandloss(
        self, df: pd.DataFrame, valid_companies: Set[str]
    ) -> pd.DataFrame:
        """
        ...
        for idx, row in df[
            ...
            if not dup_mask.loc[idx]:  # This is the last one, it will be kept
                self.log_failure(
                    row["company_id"],
                    row["year"],
                    "year",
                    "Duplicate annual record found. Keeping last occurrence (DQ-02).",
                    "CRITICAL",
                )
        ...
        for idx, row in df_dedup.iterrows():
            ...
            if co_id not in valid_companies:
                self.log_failure(
                    ...
                    yr,
                    "company_id",
                    f"Orphan record: {co_id} not in companies table (DQ-03)",
                    "CRITICAL",
                )
                reject = True
            ...
            if yr == "PARSE_ERROR" or not re.match(r"^\d{4}-\d{2}$", str(yr)):
                self.log_failure(
                    co_id, yr, "year", f"Invalid year format: {yr} (DQ-07)", "CRITICAL"
                )
                reject = True
            ...
            if not (2 <= len(str(co_id)) <= 12):
                self.log_failure(
                    ...
                    yr,
                    "company_id",
                    f"Ticker length out of range: {co_id} (DQ-08)",
                    "CRITICAL",
                )
                reject = True
            ...
            if (
                ...
                if abs(opm_pct - calc_opm) >= 1.0:
                    self.log_failure(
                        ...
                        yr,
                        "opm_percentage",
                        f"OPM mismatch: reported {opm_pct}%, calculated {calc_opm:.2f}% (DQ-05)",
                        "WARNING",
                    )
            ...
            if co_id not in self.financial_sectors:
                if pd.notna(sales) and sales <= 0:
                    self.log_failure(
                        ...
                        yr,
                        "sales",
                        f"Non-positive sales for non-financial company: {sales} (DQ-06)",
                        "WARNING",
                    )
            ...
            if pd.notna(tax_pct) and not (0 <= tax_pct <= 60):
                self.log_failure(
                    ...
                    yr,
                    "tax_percentage",
                    f"Out of range tax rate: {tax_pct}% (DQ-11)",
                    "WARNING",
                )
            ...
            if pd.notna(div_payout) and div_payout > 200:
                self.log_failure(
                    ...
                    yr,
                    "dividend_payout",
                    f"Dividend payout {div_payout}% exceeds 200% limit (DQ-12)",
                    "WARNING",
                )
            ...
            if pd.notna(net_prof) and pd.notna(eps):
                if net_prof > 0 and eps <= 0:
                    self.log_failure(
                        ...
                        yr,
                        "eps",
                        f"EPS is non-positive ({eps}) while net profit is positive ({net_prof}) (DQ-14)",
                        "WARNING",
                    )
        ...
        for idx, row in df_dedup.iterrows():
            ...
            if pd.notna(tot_assets) and tot_assets > 0 and pd.notna(tot_liab):
                ...
                if pct_diff >= 0.01:
                    self.log_failure(
                        ...
                        yr,
                        "total_assets",
                        f"Balance sheet mismatch: assets {tot_assets}, liabilities {tot_liab} (diff {pct_diff*100:.2f}%) (DQ-04)",
                        "WARNING",
                    )
            ...
            if pd.notna(fixed_ast) and fixed_ast < 0:
                self.log_failure(
                    ...
                    yr,
                    "fixed_assets",
                    f"Negative fixed assets coerced to 0: {fixed_ast} (DQ-10)",
                    "WARNING",
                )
                row["fixed_assets"] = 0
            ...
            if pd.notna(tot_assets) and pd.notna(tot_liab) and tot_assets != tot_liab:
                self.log_failure(
                    ...
                    yr,
                    "total_assets",
                    f"BSE/ASE balance mismatch (strict): assets {tot_assets} != liabilities {tot_liab} (DQ-15)",
                    "INFO",
                )
            ...
            if pd.notna(net_cf):
                if abs(net_cf - calc_net_cf) > 10:  # 10 Cr tolerance
                    self.log_failure(
                        ...
                        yr,
                        "net_cash_flow",
                        f"Net Cash Flow mismatch: reported {net_cf}, calculated {calc_net_cf} (DQ-09)",
                        "WARNING",
                    )
        ...
        for idx, row in df_dedup.iterrows():
            ...
            if co_id not in valid_companies:
                self.log_failure(
                    ...
                    yr,
                    "company_id",
                    f"Orphan record in documents: {co_id} (DQ-03)",
                    "CRITICAL",
                )
                reject = True
            ...
            if pd.notna(url):
                ...
                is_valid_format = (
                    re.match(r"^https?://[^\s/$.?#].[^\s]*$", url_str, re.IGNORECASE)
                    is not None
                )
                if not is_valid_format:
                    self.log_failure(
                        ...
                        yr,
                        "Annual_Report",
                        f"Invalid URL format: {url_str} (DQ-13)",
                        "WARNING",
                    )
                    ...
                    if (
                        url_str not in self._checked_urls
                        and len(self._checked_urls) < 5
                    ):
                        self._checked_urls.add(url_str)
                        try:
                            res = requests.head(
                                url_str, timeout=1.0, allow_redirects=True
                            )
                            if res.status_code == 404:
                                self.log_failure(
                                    ...
                                    yr,
                                    "Annual_Report",
                                    f"URL returned 404: {url_str} (DQ-13)",
                                    "WARNING",
                                )
                            ...
                            self.log_failure(
                                ...
                                yr,
                                "Annual_Report",
                                f"URL connection failed: {url_str} - Error: {e} (DQ-13)",
                                "WARNING",
                            )
        ...
        for idx, row in df.iterrows():
            ...
            if co_id not in valid_companies:
                self.log_failure(
                    ...
                    "N/A",
                    "company_id",
                    f"Orphan record in analysis: {co_id} (DQ-03)",
                    "CRITICAL",
                )
                continue
            if not (2 <= len(str(co_id)) <= 12):
        ...
        for idx, row in df.iterrows():
            ...
            if co_id not in valid_companies:
                self.log_failure(
                    ...
                    "N/A",
                    "company_id",
                    f"Orphan record in prosandcons: {co_id} (DQ-03)",
                    "CRITICAL",
                )
                continue
            if not (2 <= len(str(co_id)) <= 12):
        ...
        for co in valid_companies:
            ...
            if min_cnt < 5:
                self.log_failure(
                    ...
                    "N/A",
                    "coverage",
                    f"Company has low coverage: P&L {pl_cnt}yrs, BS {bs_cnt}yrs, CF {cf_cnt}yrs (under 5 years) (DQ-16)",
                    "WARNING",
                )
    ...
    def save_failures(self, filepath: str):
        ...
        print(
            f"Validation failures logged to {filepath}. Total violations: {len(self.failures)}"
        )
